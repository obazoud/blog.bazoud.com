...
describe "Category"
	"A page":
		...
		"can open a URL on localhost":
			topic: t (page) ->
				page.open "http://127.0.0.1:10113/category/Uncategorized", (status) =>
			...
			"and the page, once it loads,":
				...
				"has a title":
					...
					"which is correct": (title) ->
						assert.equal title, "Le blog d'Olivier » Catégorie Uncategorized"
				...
				"has page title":
					...
					"which is correct": (title) ->
						assert.equal title, "Catégorie Uncategorized"

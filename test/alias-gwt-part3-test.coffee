...
describe "Alias article GWT"
	"A page":
		...
		"can open a URL on localhost":
			topic: t (page) ->
				page.open "http://localhost:10113/post/2008/07/31/Can-I-speed-up-the-GWT-compiler-Part-III", (status) =>
			...
			"and the page, once it loads,":
				...
				"has a title":
					...
					"which is correct": (title) ->
						assert.equal title, "Le blog d'Olivier » How to speed up the GWT compiler ? (Part III)"
				...
				"has page title":
					...
					"which is correct": (title) ->
						assert.equal title, "How to speed up the GWT compiler ? (Part III)"
